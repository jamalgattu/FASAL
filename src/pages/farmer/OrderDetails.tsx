...
export default function FarmerOrderDetails() {
  ...
  React.useEffect(() => {
    if (!id) return;
    farmerService.getOrderById(id).then(setOrder);
  }, [id]);
  ...
  return (
    <div className="max-w-3xl">
      <Link to="/farmer/orders" className="inline-flex items-center gap-1.5 text-sm text-stone-500 hover:text-stone-700 mb-3">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card className="p-4 md:col-span-2">
          ...
          <dl className="grid grid-cols-2 gap-y-3 text-sm">
            ...
            <dd className="text-stone-900 font-medium text-right">{order.buyerName}</dd>
